...
public class BeanTools {
	...
	public List<ReviewBean> copyEntityListToReviewBeanList(List<Review> reviewList) {
		List<ReviewBean> beanList = new ArrayList<>();
		for (Review review : reviewList) {
			ReviewBean bean = new ReviewBean();
			bean.setReviewId(review.getReviewId());
			bean.setStar(review.getStar());
			bean.setTitle(review.getTitle());
			bean.setContent(review.getContent());
			if (review.getItem() != null) {
				bean.setItemId(review.getItem().getId());
			}
			beanList.add(bean);
		}
		return beanList;
